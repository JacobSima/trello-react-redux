...
const AddBoardModal = ({setIsAddBoardModalOpen}) => {
  ...
  // Submit Creation or Edition of Board
  const onSubmit = () => {
    // run form validation
    const data = newColumns?.map(col => col?.name);
    const boardIndex = boards.length;
    const a = isNullOrUndefinedOrEmpty(name);
    const b = !isStringArrayValide(data)
    if(a || b ){
      // set noty
    ...
    // dispatch(boardSlice.actions.updateActiveBoard(boardIndex ));
    dispatch(boardSlice.actions.setActiveBoard(boardIndex ));
